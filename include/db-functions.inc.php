<?php
	...
	*/
     ...
     */
	function dbDelete($table, $where=array()){
		global $con;
		try{
    ...
    function getChallengesForCurrentTeam($teamId){
        $returnArray = array();
        $challenges = dbRaw("SELECT t.name, t.image, t.enabled, i.id, t.author, i.username, i.password FROM challenge_templates AS t INNER JOIN challenge_instance AS i ON t.id=i.parentId WHERE i.teamId = ".$teamId." AND t.enabled=1");
